...
export class Room {
  ...
  // An abandoned/frozen bean can't report its own death (its tab is paused). The still-awake client
  // that took it over and simulated it falling out reports it here, so last-man-standing resolves and
  // the frozen bean isn't crowned the survivor. First awake reporter wins (the result guard dedupes).
  reportPeerOut(reporter: Player, targetId: string, survivalMs: number): void {
    if (this.phase !== "running") return;
    if (!this.players.includes(reporter)) return; // only match participants may report
    const target = this.players.find((p) => p.id === targetId);
    if (!target || target.result) return; // unknown or already resolved
    target.result = {
      survivalMs: clamp(survivalMs, 0, this.elapsedSinceGo()),
      finished: false,
      reason: "died",
      reportedAt: Date.now(),
    };
    target.pose = null; // stop fanning its frozen pose
    log(`[${this.mode}] match ${this.matchId} peer-out ${target.nick || target.id} (by ${reporter.nick || reporter.id})`);
    this.maybeEnd();
  }
