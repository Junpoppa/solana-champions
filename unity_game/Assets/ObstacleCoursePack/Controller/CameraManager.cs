...
public class CameraManager : MonoBehaviour {
	...
	public float mouseSpeed = 2f; //Speed ​​at which we rotate the camera with the mouse (web Settings slider can override via WebBridge)
	...
	float pendingX; //Mouse delta accumulated per render frame, consumed each physics step
	float pendingY;
	...
	{ //Function that rotates the camera correctly
		...
		lookAngle = Mathf.Repeat(lookAngle, 360f); //Keep yaw wrapped: unbounded += loses float precision after long panning (yaw "jams")
	...
	private void Update()
	{
		//Mouse axes are per-RENDER-frame deltas. Reading them in FixedUpdate drops deltas when render fps
		//outpaces the physics rate (and double-counts when it lags) — intermittently "eats" the pan and the
		//camera sticks. Accumulate here, consume in FixedUpdate.
		pendingX += Input.GetAxis("Mouse X");
		pendingY += Input.GetAxis("Mouse Y");

		//Browser Esc (or tab-out) drops pointer lock; re-engage on the next click so mouse-look
		//doesn't stay edge-clamped for the rest of the match.
	...
	{//Function that correctly rotates the camera based on the joystick / mouse and follows the player (the delta time is sent to be independent of the fps)
		float h = pendingX; pendingX = 0f;
		float v = pendingY; pendingY = 0f;
